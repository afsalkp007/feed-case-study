...
class FeedImageDataLoaderCacheDecoratorTests: XCTestCase, FeedImageDataLoaderTestCase {
  ...
  func test_cancelLoadImageData_cancelsLoadedTask() {
    let url = anyURL()
    let (sut, loader) = makeSUT()
    ...
    XCTAssertEqual(loader.cancelledURLs, [url], "Expected to cancel URL on cancel load")
  }
  ...
  func test_loadImageData_doesNotCachesDataOnLoaderFailure() {
    let cache = CacheSpy()
    let url = anyURL()
    let (sut, loader) = makeSUT(cache: cache)
