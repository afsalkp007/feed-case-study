...
class CodableFeedStore {
  ...
  func retrieve(completion: @escaping FeedStore.RetrievalCompletion) {
    ...
    completion(.found(items: cache.feed, timestamp: cache.timestamp))
  }
...
class CodableFeedStoreTests: XCTestCase {
  ...
  func test_retrieveAfterInsertingToEmptyCache_deliversInsertedValues() {
    ...
    sut.insert(feed, timestamp: timestamp) { insertionError in
      ...
      sut.retrieve { retrievelResult in
        switch retrievelResult {
        case let .found(items: retrievedFeed, timestamp: retrievedTimestamp):
          XCTAssertEqual(retrievedFeed, feed)
          XCTAssertEqual(retrievedTimestamp, timestamp)
          
        default:
          XCTFail("Expected found result with feed \(feed) and timestamp \(timestamp), got \(retrievelResult) instead")
        }
